...
function menu_tab() {
    $menu_title = esc_html__('Shopline Options', 'shopline');
    add_theme_page( esc_html__( 'Shopline', 'shopline' ), $menu_title, 'edit_theme_options', 'thunk_started',array($this,'tab_page'));
...
wp_enqueue_script('shopline-admin-load', get_template_directory_uri() . '/inc/th-option/assets/js/th-options.js',array( 'jquery', 'updates' ),'1', true);
...
$data = apply_filters(
    ...
    wp_localize_script( 'shopline-admin-load', 'THAdmin', $data); 
...
function tab_constant(){
    ...
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'shopline'), esc_html($theme_data->get( 'Name' )), $theme_data->get( 'Version' ) ),
    'welcome_desc' => esc_html($theme_data->get( 'Name' ).' is a full featured E-commerce WordPress theme build specially for WooCommerce.', 'shopline' ),
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','shopline'),
       'post_content' => '',
       'post_status' => 'publish',
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'shopline' );
            ...
            if ( ! is_plugin_active( $plugin_init ) ){
                    ...
                    if ( ! $status ) {
                        ...
                        $button_txt = esc_html__( 'Activate Now', 'shopline' );
                    }
                }
